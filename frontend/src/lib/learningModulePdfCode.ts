import type { LearningModuleAudience, LearningModuleContentType } from "@/lib/learningModulePdfApi";
...
export function generateLearningModulePdfSuffix(length = 6) {
...
export async function generateLearningModulePdfBaseCode(params: {
  duration: "D1" | "D2" | "D3";
...
export async function generateLearningModulePdfFullCode(params: {
  duration: "D1" | "D2" | "D3";
  audience: LearningModuleAudience[];
  contentTypes: LearningModuleContentType[];
  suffix?: string;
  ...
  const suffix = params.suffix || generateLearningModulePdfSuffix(6);
  return `${base}-${suffix}`;
}
