export type CountryCallingCodeOption = {
  value: string; // dial code in E.164 prefix form, e.g. "+62"
  label: string; // e.g. "Indonesia (+62)"
};

const RAW_COUNTRY_CALLING_CODES: CountryCallingCodeOption[] = [
...
function extractCountryName(label: string) {
  const i = label.lastIndexOf("(+");
  const name = i > 0 ? label.slice(0, i).trim() : label.trim();
  return name;
}

function buildCountryCallingCodes(input: CountryCallingCodeOption[]): CountryCallingCodeOption[] {
  const codeToCountries = new Map<string, string[]>();
  const codeOrder: string[] = [];

  for (const opt of input) {
    const code = (opt.value || "").trim();
    const name = extractCountryName(opt.label);
    if (!code) continue;

    if (!codeToCountries.has(code)) {
      codeToCountries.set(code, []);
      codeOrder.push(code);
    }

    const arr = codeToCountries.get(code)!;
    const lower = name.toLowerCase();
    if (name && !arr.some((x) => x.toLowerCase() === lower)) {
      arr.push(name);
    }
  }

  const merged: CountryCallingCodeOption[] = codeOrder.map((code) => {
    const countries = codeToCountries.get(code) || [];
    const label = `${countries.join(" / ")} (${code})`;
    return { value: code, label };
  });

  merged.sort((a, b) => {
    const aCountries = (codeToCountries.get(a.value) || []).map((x) => x.toLowerCase());
    const bCountries = (codeToCountries.get(b.value) || []).map((x) => x.toLowerCase());

    const aAsia = aCountries.some((c) => ASIA_COUNTRIES.has(c));
    const bAsia = bCountries.some((c) => ASIA_COUNTRIES.has(c));
    if (aAsia !== bAsia) return aAsia ? -1 : 1;
...
export const COUNTRY_CALLING_CODES: CountryCallingCodeOption[] = buildCountryCallingCodes(RAW_COUNTRY_CALLING_CODES);
