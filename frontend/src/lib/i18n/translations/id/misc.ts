export const profile = {
  title: "Profil",
  companyProfile: "Profil Perusahaan",
  companyProfileDesc: "Kelola profil perusahaan dan logo.",
  companyInfo: "Info Perusahaan",
  companyInfoDesc: "Informasi dasar perusahaan.",
  companyPhoto: "Foto Perusahaan",
  companyPhotoDesc: "Unggah logo atau foto perusahaan.",
  uploadPhoto: "Unggah Foto",
  noPhoto: "Belum Ada",
  companyNamePlaceholder: "Perusahaan",
  companyCode: "Company Code",
  companyCodePlaceholder: "Masukkan company code",
  resetCompanyCode: "Reset",
  photoHint: "Disarankan gambar kotak",
  photoStorageNote: "",
  companyPhotoUploaded: "Foto perusahaan berhasil diunggah",
  editProfile: "Ubah Profil",
  editBiography: "Ubah Biografi",
  personalInfo: "Informasi Pribadi",
  changeAvatar: "Ganti Avatar",
  updateSuccess: "Profil berhasil diperbarui",
  changePassword: "Ubah Password",
} as const;

export const settings = {
  title: "Pengaturan",
  general: "Umum",
  appearance: "Tampilan",
  notifications: "Notifikasi",
  security: "Keamanan",
  language: "Bahasa",
  timezone: "Zona Waktu",
  theme: "Tema",
} as const;

export const errors = {
  general: "Terjadi kesalahan",
  notFound: "Tidak ditemukan",
  unauthorized: "Tidak terotorisasi",
  forbidden: "Dilarang",
  serverError: "Kesalahan server",
  networkError: "Kesalahan jaringan",
  validationError: "Kesalahan validasi",
  sessionExpired: "Sesi berakhir, silakan login kembali",
} as const;

export const confirmation = {
  delete: "Apakah Anda yakin ingin menghapus item ini?",
  unsavedChanges: "Anda memiliki perubahan yang belum disimpan. Yakin ingin keluar?",
  logout: "Apakah Anda yakin ingin keluar?",
} as const;

export const time = {
  today: "Hari Ini",
  yesterday: "Kemarin",
  tomorrow: "Besok",
  thisWeek: "Minggu Ini",
  lastWeek: "Minggu Lalu",
  thisMonth: "Bulan Ini",
  lastMonth: "Bulan Lalu",
  thisYear: "Tahun Ini",
} as const;

export const languages = {
  en: "Inggris",
  id: "Indonesia",
  zh: "Mandarin",
  ja: "Jepang",
  ko: "Korea",
  th: "Thailand",
  vi: "Vietnam",
  ms: "Melayu",
  tl: "Filipina",
  hi: "Hindi",
} as const;
