...
export type LearningModulePdfDuration = "D1" | "D2" | "D3";

export type LearningModulePdfShareScope = "GENERAL" | "COMPANY_ONLY";
...
export interface LearningModulePdfRequest {
  title: string;
  duration: LearningModulePdfDuration;
  shareScope: LearningModulePdfShareScope;
...
export interface LearningModulePdfResponse {
  id: number;
  code: string;
  title: string;
  duration: LearningModulePdfDuration;
  shareScope?: LearningModulePdfShareScope | null;
  createdByCompanyName?: string | null;
...
const BASE = "/learning-module/pdfs";

export const learningModulePdfApi = {
  list: (params: {
    page: number;
    size: number;
    sortBy?: string;
    ...
    return apiCall<PageResponse<LearningModulePdfResponse>>(`${BASE}?${qs.toString()}`);
  },
  ...
  getById: (id: number, params?: { companyName?: string }) => {
    ...
    return apiCall<LearningModulePdfResponse>(url);
  },

  create: (payload: LearningModulePdfRequest) =>
    apiCall<LearningModulePdfResponse>(BASE, {
      method: "POST",
      body: JSON.stringify(payload),
    }),

  update: (id: number, payload: LearningModulePdfRequest, params?: { companyName?: string }) => {
    const qs = new URLSearchParams();
    if (params?.companyName) qs.set("companyName", params.companyName);
    const url = qs.toString() ? `${BASE}/${id}?${qs.toString()}` : `${BASE}/${id}`;
    return apiCall<LearningModulePdfResponse>(url, {
