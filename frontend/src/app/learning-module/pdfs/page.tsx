...
import {
  ...
  type LearningModulePdfResponse,
...
export default function LearningModulePdfsPage() {
  const { t } = useLanguage();
  ...
  const formatAudience = React.useCallback(
    (values: string[] | undefined | null) => {
      const map: Record<string, string> = {
        GENERAL: t("learningModule.pdf.audienceGeneral"),
        TOP_LEADER: t("learningModule.pdf.audienceTopLeader"),
        LEADER: t("learningModule.pdf.audienceLeader"),
        TOP_AGENT: t("learningModule.pdf.audienceTopAgent"),
        AGENT: t("learningModule.pdf.audienceAgent"),
        NEW_LEADER: t("learningModule.pdf.audienceNewLeader"),
        NEW_AGENT: t("learningModule.pdf.audienceNewAgent"),
      };
  ...
  const formatContentTypes = React.useCallback(
    (values: string[] | undefined | null) => {
      const map: Record<string, string> = {
        GENERAL: t("learningModule.pdf.contentGeneral"),
        LEADERSHIP: t("learningModule.pdf.contentLeadership"),
        MOTIVATION_COACH: t("learningModule.pdf.contentMotivationCoach"),
        PERSONAL_SALES: t("learningModule.pdf.contentPersonalSales"),
        RECRUITMENT: t("learningModule.pdf.contentRecruitment"),
        PRODUCT: t("learningModule.pdf.contentProduct"),
        LEGAL_COMPLIANCE: t("learningModule.pdf.contentLegalCompliance"),
        OPERATION: t("learningModule.pdf.contentOperation"),
      };
      return (values ?? []).map((v) => map[v] ?? v).join(", ");
    },
    [t]
  ...
  const formatShare = React.useCallback(
    (v: string | undefined | null) => {
      if (v === "COMPANY_ONLY") return t("learningModule.pdf.pdfShareCompanyOnly");
      return t("learningModule.pdf.pdfShareGeneral");
    },
    [t]
  );
  ...
  const [items, setItems] = React.useState<LearningModulePdfResponse[]>([]);
  ...
  const load = React.useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await learningModulePdfApi.list({
        page,
        size: pageSize,
  ...
  }, [page, pageSize, companyName, appliedTitle, appliedDuration, appliedCreator, appliedAudiences, appliedContentTypes]);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await learningModulePdfApi.remove(id, { companyName: companyName || undefined });
      load();
    } catch (e: any) {
  ...
  const onShowCode = (item: LearningModulePdfResponse) => {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-4 p-4 md:p-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl font-semibold md:text-2xl">{t("learningModule.pdf.title")}</h1>
          <p className="text-sm text-muted-foreground">{t("learningModule.pdf.subtitle")}</p>
        </div>
        <Button asChild>
          <Link href="/learning-module/pdfs/new">{t("learningModule.pdf.create")}</Link>
        </Button>
      </div>
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <CardTitle className="text-base">{t("learningModule.pdf.list")}</CardTitle>
        ...
        <CardContent className="space-y-3">
              ...
              <CardContent className="pt-4">
                <form className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3" onSubmit={onSearch}>
                  <div className="space-y-1">
                    ...
                    <Input
                      placeholder={tt("learningModule.pdf.titlePlaceholder", "Cari judul...")}
                      value={draftTitle}
                  ...
                  <div className="space-y-1">
                    ...
                    <Select value={draftDuration || "ALL"} onValueChange={setDraftDuration}>
                      <SelectTrigger className="h-8 text-sm">
                        <SelectValue placeholder={tt("learningModule.pdf.duration", "Pilih durasi")} />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      placeholder={tt("learningModule.pdf.createdByPlaceholder", "Cari company...")}
                      value={draftCreator}
                  ...
                  <div className="space-y-1">
                    ...
                    <MultiSelect<LearningModuleAudience>
                      ...
                      placeholder={tt("learningModule.pdf.audience", "Pilih audiens")}
                  ...
                  <div className="space-y-1">
                    ...
                    <MultiSelect<LearningModuleContentType>
                      ...
                      placeholder={tt("learningModule.pdf.contentTypes", "Pilih tipe konten")}
          ...
          <div className="rounded-md border">
            <div className="hidden md:grid grid-cols-12 gap-2 border-b bg-muted/40 px-3 py-2 text-xs font-medium text-muted-foreground">
              <div className="col-span-12 md:col-span-3">{t("learningModule.pdf.pdfTitle") || "Judul"}</div>
              <div className="col-span-6 md:col-span-1">{t("learningModule.pdf.duration")}</div>
              <div className="col-span-6 md:col-span-1">{t("learningModule.pdf.pdfShare")}</div>
              <div className="col-span-12 md:col-span-3">{t("learningModule.pdf.createdBy")}</div>
              <div className="col-span-12 md:col-span-1">{t("learningModule.pdf.audience")}</div>
              <div className="col-span-12 md:col-span-2">{t("learningModule.pdf.contentTypes")}</div>
                        ...
                        {it.canEdit ? (
                          <>
                            <Button asChild variant="ghost" size="icon" className="h-8 w-8">
                              <Link href={`/learning-module/pdfs/${it.id}/edit`}>
