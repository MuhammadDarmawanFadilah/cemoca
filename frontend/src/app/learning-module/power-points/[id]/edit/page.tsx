...
const DURATION_OPTIONS: { value: LearningModulePowerPointDuration; labelKey: string }[] = [
...
export default function EditLearningModulePowerPointPage() {
  const { t } = useLanguage();
  ...
  const [duration, setDuration] = React.useState<LearningModulePowerPointDuration | "">("");
  ...
  const load = React.useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await learningModulePowerPointApi.getById(id, { companyName: companyName || undefined });
      setTitle(res.title || "");
      ...
      setExistingFilename(String(res.powerPointFilename || ""));
      setExistingUrl(
        String(
          res.powerPointUrl ||
            (res.powerPointFilename ? `/api/document-files/${res.powerPointFilename}` : "")
        )
      );
      setSuffix(generateLearningModulePowerPointSuffix(6));
    } catch (e: any) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await learningModulePowerPointApi.update(
        id,
        {
          title,
          duration: duration as LearningModulePowerPointDuration,
      ...
      setExistingFilename(String(res.powerPointFilename || uploaded.filename));
      setExistingUrl(String(res.powerPointUrl || uploaded.url));
            ...
            {step === 1 ? (
              <div className="space-y-4">
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
                  <Button type="button" variant="outline" onClick={() => router.push("/learning-module/power-points")}
                  >
                    {t("common.cancel")}
                  </Button>
