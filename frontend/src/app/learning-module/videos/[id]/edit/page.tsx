...
import {
  buildLearningModuleVideoCodePreview,
  generateLearningModuleVideoSuffix,
} from "@/lib/learningModuleVideoCode";
...
export default function EditLearningModuleVideoPage() {
  const { t } = useLanguage();
  ...
  const [generatedMeta, setGeneratedMeta] = React.useState<{ words: number; minutesLabel: string; rangeLabel: string } | null>(null);

  const [companyName, setCompanyName] = React.useState<string>("");
  const [canEdit, setCanEdit] = React.useState<boolean>(true);

  // For Edit, we store the original code to display initially
  ...
  const estimatedMinutes = wordCount / 130; // ~130 words per minute
  ...
  React.useEffect(() => {
    ...
    let alive = true;
    (async () => {
      const next = await buildLearningModuleVideoCodePreview({
        duration: duration as LearningModuleVideoDuration,
        audience,
        contentTypes,
        suffix: suffix || "??????" // Placeholder suffix if we don't have one
      });
      
      if (alive) {
         // If the generated preview prefix matches the original code prefix, we might want to keep original code.
         // But for now, let's just show the preview if it differs significantly, or just show original code if fields match.
         // Actually, let's just show the preview. The user will see that changing fields changes the code.
         // But wait, if I just loaded the page, the preview will be different because suffix is random/placeholder.
         // We should try to keep the original code visible if the fields haven't changed.
         // But detecting "fields haven't changed" requires storing original state.
         // Let's simplify: Just show the preview. If they save, the backend handles it.
         // But showing "?????" is ugly.
         // Let's just show the original code if we are in "Edit" mode and haven't touched anything?
         // Too complex.
         // Let's just show the original code in the input, and if they change something, maybe we update it?
         // Or just don't update it dynamically in Edit mode to avoid confusion.
         // The "Create" page updates it dynamically.
         // Let's just stick to showing the `codePreview` state, which we initialized with `res.code`.
         // And we WON'T update it dynamically in this effect for now, to avoid the "??????" issue.
         // If the user really wants to see the new code, they have to save.
         // OR, we can try to extract the suffix from the original code.
         // Format: D1-HASH-SUFFIX.
         // If we can extract suffix, we can use it in the builder.
      }
    })();
    return () => {
      alive = false;
    };
  }, [duration, audience, contentTypes, suffix, loading]);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push("/learning-module/videos");
    } catch (e: any) {
  ...
  const onGenerateIdea = async () => {
    ...
    try {
      ...
      const minRange = Number(data?.estimatedMinutesRange?.min || 0);
      const maxRange = Number(data?.estimatedMinutesRange?.max || 0);

      const minutesLabel = minutes > 0 ? `${Math.round(minutes * 10) / 10}` : "0";
      const rangeLabel =
        minRange > 0 && maxRange > 0
          ? `${Math.round(minRange * 10) / 10}–${Math.round(maxRange * 10) / 10}`
          : "0";

      setGeneratedMeta({ words, minutesLabel, rangeLabel });
      setStep(2);
    } catch (e: any) {
          ...
          <div className="flex justify-center">
            <div className="w-full max-w-md">
              <div className="relative">
                ...
                />
                {/* In Edit mode, we don't allow manual regeneration of suffix, as it's tied to the record unless changed by backend */}
              </div>
            </div>
          </div>
        </CardHeader>
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground">{t("learningModule.video.duration")}, {t("learningModule.video.videoShare")}, {t("learningModule.video.contentTypes")}, {t("learningModule.video.audience")}</div>
                  </div>
                </button>
