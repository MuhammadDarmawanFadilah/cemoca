...
export default function ReportDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const loadItems = useCallback(async (reportId: number, pageNum: number, statusFilter: string, searchQuery: string, showLoading = true) => {
    try {
      ...
      const reportWithItems = await pdfReportAPI.getPdfReport(
        reportId, 
        pageNum, 
        ITEMS_PER_PAGE, 
        backendStatus || undefined,
        searchQuery || undefined,
        backendWaStatus || undefined
      );
      setCurrentReport(reportWithItems);
    } catch {
      toast.error(t("reportPdf.loadDataFailed"));
    } finally {
  ...
  const loadReport = async () => {
    try {
      ...
      const report = await pdfReportAPI.getPdfReport(reportId, 0, ITEMS_PER_PAGE);
      setCurrentReport(report);
    } catch {
      toast.error(t("reportPdf.loadDetailFailed"));
      router.push("/report-pdf/personal-letter");
    } finally {
      setLoading(false);
    }
  };
  ...
  const refreshStatus = async (showToast = true) => {
    ...
    try {
      ...
      const updated = await pdfReportAPI.getPdfReport(
        currentReport.id, 
        page, 
        ITEMS_PER_PAGE, 
        backendStatus || undefined, 
      ...
      if (showToast) toast.success(t("reportPdf.statusUpdated"));
    } catch {
      if (showToast) toast.error(t("reportPdf.refreshFailed"));
    } finally {
      setRefreshing(false);
    }
  };
  ...
  const handleExportExcel = async () => {
    ...
    try {
      setExporting(true);
      await pdfReportAPI.exportToExcel(currentReport.id, currentReport.reportName);
      toast.success(t("reportPdf.exportSuccess"));
    } catch {
      toast.error(t("reportPdf.exportFailed"));
    } finally {
      setExporting(false);
    }
  };
  ...
  const handleStartWaBlast = async () => {
    ...
    try {
      ...
      await pdfReportAPI.startWaBlast(currentReport.id);
      toast.success(t("reportPdf.waBlastStarted"));
      await refreshStatus(false);
    } catch {
      toast.error(t("reportPdf.waBlastFailed"));
    } finally {
      setWaBlasting(false);
    }
  };
    ...
    switch (status) {
      case "DONE": return (
        <div className={`${base} bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400`}>
          <span className="flex items-center gap-0.5"><CheckCircle2 className="h-2.5 w-2.5" /> {t("reportPdf.done")}</span>
      ...
      case "FAILED": return (
        <div className={`${base} bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-400`}>
          <span className="flex items-center gap-0.5"><XCircle className="h-2.5 w-2.5" /> {t("reportPdf.failed")}</span>
        </div>
      );
      case "PROCESSING": return (
        <div className={`${base} bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-400`}>
          <span className="flex items-center gap-0.5"><RefreshCw className="h-2.5 w-2.5 animate-spin" /> {t("reportPdf.processing")}</span>
        </div>
      );
      default: return (
        <div className={`${base} bg-slate-100 dark:bg-slate-800`}>
          <span className="flex items-center gap-0.5"><Clock className="h-2.5 w-2.5" /> {t("reportPdf.pending")}</span>
        </div>
      );
    }
  };
  ...
  const getWaStatusBadge = (status: string | undefined, timestamp?: string) => {
    ...
    switch (status) {
      case "SENT": return (
        <div className={`${base} bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400`}>
          <span className="flex items-center gap-0.5"><CheckCircle2 className="h-2.5 w-2.5" /> {t("reportPdf.sent")}</span>
          {timestamp && <span className="text-[8px] opacity-75">{formatTime(timestamp)}</span>}
        </div>
      );
      case "FAILED": return (
  ...
  const resendWa = async (itemId: number) => {
    if (!currentReport) return;
    try {
      toast.info(t("reportPdf.resending"));
      await pdfReportAPI.resendWa(currentReport.id, itemId);
      toast.success(t("reportPdf.waSent"));
      await loadReport();
    } catch { toast.error(t("reportPdf.resendFailed")); }
  };
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
        <div className="border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
            <div>
              <h1 className="text-lg font-semibold text-slate-900 dark:text-slate-100">{t("reportPdf.title")}</h1>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{t("reportPdf.detailReport")}</p>
            </div>
            <Button size="sm" variant="outline" onClick={backToHistory} className="h-8 text-xs">
              <ArrowLeft className="h-3.5 w-3.5 mr-1.5" /> {t("reportPdf.back")}
            </Button>
          </div>
        </div>
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
          <div className="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 p-4 sm:p-6 text-center py-8">
            <div className="text-slate-500">{t("reportPdf.reportNotFound")}</div>
            <Button variant="outline" size="sm" className="mt-4" onClick={backToHistory}>
              <ArrowLeft className="h-3.5 w-3.5 mr-1" /> {t("reportPdf.back")}
            </Button>
          </div>
        </div>
      </div>
      ...
      <div className="border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{t("reportPdf.detailReportPdf")}</p>
          </div>
          <div className="flex items-center gap-2">
            {(currentReport.waPendingCount || 0) > 0 && (
              <Button size="sm" variant="default" onClick={handleStartWaBlast} disabled={waBlasting} className="h-8 text-xs bg-green-600 hover:bg-green-700">
                ...
                {t("reportPdf.sendWaPending")} ({currentReport.waPendingCount})
              </Button>
            )}
        ...
        <div className="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 p-4 sm:p-6 space-y-4">
          ...
          {isComplete && (
            <div className="bg-gradient-to-r from-emerald-50 to-green-50 dark:from-emerald-950/30 dark:to-green-950/30 rounded-lg p-4 text-center border border-emerald-200 dark:border-emerald-800">
              ...
              <h3 className="text-lg font-semibold text-emerald-700 dark:text-emerald-400">{t("reportPdf.processComplete")}</h3>
              <p className="text-sm text-emerald-600 dark:text-emerald-500">{t("reportPdf.allPdfWaSent")}</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2">
            ...
            <div className="bg-blue-50 dark:bg-blue-950/30 rounded p-3 text-center">
              ...
              <div className="text-[10px] text-slate-500 flex items-center justify-center gap-1">
                <FileText className="h-3 w-3" /> {t("reportPdf.pdfPending")}
              </div>
            </div>
            <div className="bg-amber-50 dark:bg-amber-950/30 rounded p-3 text-center">
              ...
              <div className="text-[10px] text-slate-500 flex items-center justify-center gap-1">
                <FileText className="h-3 w-3" /> {t("reportPdf.pdfProcessing")}
              </div>
            </div>
            <div className="bg-emerald-50 dark:bg-emerald-950/30 rounded p-3 text-center">
              ...
              <div className="text-[10px] text-slate-500 flex items-center justify-center gap-1">
                <FileText className="h-3 w-3" /> {t("reportPdf.pdfSuccess")}
              </div>
            </div>
            <div className="bg-red-50 dark:bg-red-950/30 rounded p-3 text-center">
              ...
              <div className="text-[10px] text-slate-500 flex items-center justify-center gap-1">
                <FileText className="h-3 w-3" /> {t("reportPdf.pdfFailed")}
              </div>
            </div>
            <div className="bg-green-50 dark:bg-green-950/30 rounded p-3 text-center">
              ...
              <div className="text-[10px] text-slate-500 flex items-center justify-center gap-1">
                <MessageSquare className="h-3 w-3" /> {t("reportPdf.waSent")}
              </div>
            </div>
            <div className="bg-yellow-50 dark:bg-yellow-950/30 rounded p-3 text-center">
              ...
              <div className="text-[10px] text-slate-500 flex items-center justify-center gap-1">
                <MessageSquare className="h-3 w-3" /> {t("reportPdf.waPending")}
              </div>
            </div>
            <div className="bg-red-50 dark:bg-red-950/30 rounded p-3 text-center">
              <div className="text-xl font-bold text-red-600 dark:text-red-400">{currentReport.waFailedCount || 0}</div>
              <div className="text-[10px] text-slate-500 flex items-center justify-center gap-1">
                <MessageSquare className="h-3 w-3" /> {t("reportPdf.waFailed")}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1">
              <div className="flex justify-between text-[10px] text-slate-500">
                <span>{t("reportPdf.progressPdf")}</span>
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-[10px] text-slate-500">
                <span>{t("reportPdf.progressWa")}</span>
          ...
          <div className="flex flex-col sm:flex-row gap-2 items-center justify-between">
            <div className="flex flex-col sm:flex-row gap-2 flex-1">
              <div className="relative flex-1 sm:max-w-xs">
                ...
                <Input
                  placeholder={t("reportPdf.searchNamePhone")}
                  value={searchInput}
              ...
              <Button variant="outline" size="sm" className="h-8 text-xs" onClick={handleSearch}>
                <Search className="h-3.5 w-3.5 mr-1" /> {t("reportPdf.search")}
              </Button>
              <Select value={filter} onValueChange={(v: FilterType) => handleFilterChange(v)}>
                ...
                <SelectContent>
                  <SelectItem value="all" className="text-xs">{t("reportPdf.all")} ({currentReport.totalRecords})</SelectItem>
                  <SelectItem value="DONE" className="text-xs">✓ {t("reportPdf.pdfDone")} ({currentReport.successCount})</SelectItem>
                  <SelectItem value="FAILED" className="text-xs">✗ {t("reportPdf.pdfFailed")} ({currentReport.failedCount})</SelectItem>
                  <SelectItem value="PROCESSING" className="text-xs">⟳ {t("reportPdf.pdfProcessing")} ({processingCount})</SelectItem>
                  <SelectItem value="PENDING" className="text-xs">◷ {t("reportPdf.pdfPending")} ({pendingCount})</SelectItem>
                  <SelectItem value="wa-SENT" className="text-xs">✓ {t("reportPdf.waSent")} ({currentReport.waSentCount || 0})</SelectItem>
                  <SelectItem value="wa-PENDING" className="text-xs">◷ {t("reportPdf.waPending")} ({currentReport.waPendingCount || 0})</SelectItem>
                  <SelectItem value="wa-FAILED" className="text-xs">✗ {t("reportPdf.waFailed")} ({currentReport.waFailedCount || 0})</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <Button variant="ghost" size="sm" className="h-8 text-xs" onClick={() => refreshStatus(true)} disabled={refreshing}>
              <RefreshCw className={`h-3.5 w-3.5 mr-1 ${refreshing ? "animate-spin" : ""}`} /> {t("reportPdf.refresh")}
            </Button>
          </div>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-slate-50 dark:bg-slate-800">
                        ...
                        <TableHead className="text-[10px] font-medium w-24">PDF</TableHead>
                        <TableHead className="text-[10px] font-medium w-24">WA</TableHead>
                        <TableHead className="text-[10px] font-medium">{t("reportPdf.actions")}</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {items.map(item => (
                        <TableRow key={item.id} className={item.status === "FAILED" || item.waStatus === "FAILED" ? "bg-red-50/30 dark:bg-red-950/10" : ""}>
                          ...
                          <TableCell>{getStatusBadge(item.status, item.pdfGeneratedAt)}</TableCell>
                          <TableCell>{getWaStatusBadge(item.waStatus, item.waSentAt)}</TableCell>
                          <TableCell>
                            <div className="flex items-center gap-1">
                              {item.pdfUrl && (
                                <>
                                  <Button variant="ghost" size="sm" className="h-6 px-1.5 text-[10px]" onClick={() => setPreviewPdf({ name: item.name, url: item.pdfUrl! })}>
                                    <Eye className="h-3 w-3" />
                                  </Button>
                                  <Button variant="ghost" size="sm" className="h-6 px-1.5 text-[10px]" onClick={() => window.open(item.pdfUrl, '_blank')}>
                                    <ExternalLink className="h-3 w-3" />
                                  </Button>
                                </>
                              )}
                              {item.waStatus === "FAILED" && item.pdfUrl && (
                                <Button variant="ghost" size="sm" className="h-6 px-1.5 text-[10px] text-orange-600" onClick={() => resendWa(item.id)}>
                                  <RefreshCw className="h-3 w-3 mr-0.5" /> {t("reportPdf.resend")}
                                </Button>
                              )}
                            </div>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between">
                  <div className="text-xs text-slate-500">
                    {t("reportPdf.showing")} {page * ITEMS_PER_PAGE + 1} - {Math.min((page + 1) * ITEMS_PER_PAGE, totalElements)} {t("reportPdf.of")} {totalElements} {t("reportPdf.data")}
                  </div>
                  <div className="flex items-center gap-1">
          ...
          <div className="flex justify-between pt-4 border-t">
            <Button variant="outline" size="sm" className="h-8 text-xs" onClick={backToHistory}>
              <ArrowLeft className="h-3.5 w-3.5 mr-1" /> {t("reportPdf.backToHistory")}
            </Button>
            {isComplete && (
              <div className="flex items-center gap-2 text-emerald-600">
                <CheckCircle2 className="h-4 w-4" />
                <span className="text-xs font-medium">{t("reportPdf.allComplete")}</span>
              </div>
            )}
          </div>
          ...
          <Dialog open={!!previewPdf} onOpenChange={() => setPreviewPdf(null)}>
            <DialogContent className="w-[95vw] max-w-5xl h-[90vh] p-3 flex flex-col">
              <DialogHeader className="flex-shrink-0">
                <DialogTitle className="text-sm">{t("reportPdf.previewPdf")} - {previewPdf?.name}</DialogTitle>
              </DialogHeader>
              {previewPdf && (
                <div className="flex-1 flex items-center justify-center">
                  <iframe src={previewPdf.url} className="w-full h-full rounded border" />
                </div>
              )}
            </DialogContent>
          </Dialog>
