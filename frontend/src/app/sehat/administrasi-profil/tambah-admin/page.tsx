...
export default function TambahAdminPage() {
  ...
  const fetchRoles = useCallback(async () => {
    try {
      ...
      // Auto-select ADMIN role
      const adminRole = data.find((r: Role) => r.roleName === "ADMIN" || r.name === "ADMIN");
      if (adminRole) {
        setFormData(prev => ({ ...prev, roleId: adminRole.id.toString() }));
      }
    } catch (err) {
      console.error(err);
      setRoles([]);
  ...
  return (
    <div className="max-w-xl mx-auto space-y-3 p-4">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-lg font-bold">Tambah Admin</h1>
      </div>
      ...
      <Card>
        ...
        <CardContent className="pt-3">
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            {/* Role Section */}
            <div className="bg-purple-50 dark:bg-purple-950/20 border border-purple-200 dark:border-purple-800 rounded-md p-2.5">
              <div className="flex items-center gap-2">
