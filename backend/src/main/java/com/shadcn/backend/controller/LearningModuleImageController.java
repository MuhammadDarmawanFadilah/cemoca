...
import com.shadcn.backend.service.LearningModuleImageService;
...
@RestController
@RequestMapping("/api/learning-module/images")
public class LearningModuleImageController {

    @Autowired
    private LearningModuleImageService service;

    @GetMapping
    public ResponseEntity<Page<LearningModuleImageResponse>> list(
    ...
    @GetMapping("/{id}")
    public ResponseEntity<LearningModuleImageResponse> getById(
            @PathVariable Long id,
            @RequestParam(required = false) String companyName
    ) {
        Optional<LearningModuleImageResponse> res = service.getById(id, companyName);
        return res.map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
    }

    @PostMapping
    public ResponseEntity<?> create(@RequestBody LearningModuleImageRequest request) {
        try {
    ...
    public ResponseEntity<?> update(
            @PathVariable Long id,
            @RequestBody LearningModuleImageRequest request,
            @RequestParam(required = false) String companyName
    ) {
        try {
            Optional<LearningModuleImageResponse> res = service.update(id, request, companyName);
