...
import com.shadcn.backend.dto.GeminiTranslateRequest;
import com.shadcn.backend.dto.GeminiTranslateResponse;
...
public class GeminiController {
    ...
    public ResponseEntity<?> translate(@RequestBody GeminiTranslateRequest request) {
        try {
            String text = request == null ? null : request.text();
            String langCode = request == null ? null : request.targetLanguageCode();
            String langName = request == null ? null : request.targetLanguageName();
            GeminiTranslateResponse res = geminiService.translate(text, langCode, langName);
            return ResponseEntity.ok(res);
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest().body(new ErrorResponse(e.getMessage()));
        } catch (Exception e) {
