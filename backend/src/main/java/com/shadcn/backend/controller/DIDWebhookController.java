...
public class DIDWebhookController {
    ...
    @PostMapping("/scenes")
    public ResponseEntity<?> scenesWebhook(
            @RequestBody(required = false) Map<String, Object> payload,
            @RequestHeader(value = "X-DID-Webhook-Secret", required = false) String providedSecret
    ) {
        try {
            ...
            String sceneId = asString(payload.get("id"));
            String status = asString(payload.get("status"));
            String resultUrl = asString(payload.get("result_url"));
            ...
            VideoReportItem item = videoReportItemRepository.findByDidClipId(sceneId);
            if (item == null) {
                return ResponseEntity.ok().build();
            }
            ...
            logger.warn("D-ID scenes webhook handling failed: {}", e.getMessage());
            return ResponseEntity.ok().build();
        }
    }
