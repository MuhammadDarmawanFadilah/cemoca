...
import com.shadcn.backend.service.LearningModulePowerPointService;
...
@RestController
@RequestMapping("/api/learning-module/power-points")
public class LearningModulePowerPointController {

    @Autowired
    private LearningModulePowerPointService service;

    @GetMapping
    public ResponseEntity<Page<LearningModulePowerPointResponse>> list(
    ...
    @GetMapping("/{id}")
    public ResponseEntity<LearningModulePowerPointResponse> getById(
            @PathVariable Long id,
            @RequestParam(required = false) String companyName
    ) {
        Optional<LearningModulePowerPointResponse> res = service.getById(id, companyName);
        return res.map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
    }

    @PostMapping
    public ResponseEntity<?> create(@RequestBody LearningModulePowerPointRequest request) {
        try {
    ...
    public ResponseEntity<?> update(
            @PathVariable Long id,
            @RequestBody LearningModulePowerPointRequest request,
            @RequestParam(required = false) String companyName
    ) {
        try {
            Optional<LearningModulePowerPointResponse> res = service.update(id, request, companyName);
